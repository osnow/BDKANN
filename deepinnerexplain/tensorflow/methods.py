...
class AttributionMethod(object):
    ...
    """
    def __init__(self, T, X, session, A=None, keras_learning_phase=None):
...
class GradientBasedMethod(AttributionMethod):
    ...
    def get_symbolic_attribution(self):
        ...
        return tf.gradients(self.T, self.A)
...
class Saliency(GradientBasedMethod):
    ...
    def get_symbolic_attribution(self):
        ...
        return [tf.abs(g) for g in tf.gradients(self.T, self.A)]
...
class GradientXInput(GradientBasedMethod):
    ...
    def get_symbolic_attribution(self):
        # return [g * x for g, x in zip( tf.gradients(self.T, self.X), self.X if self.has_multiple_inputs else [self.X])]

        return [g * x for g, x in zip( tf.gradients(self.T, self.A), self.A if self.has_multiple_inputs else [self.A])]
...
class IntegratedGradients(GradientBasedMethod):
    ...
    def __init__(self, T, X, session,  keras_learning_phase, A=None, steps=100, baseline=None):
        self.steps = steps
        self.baseline = baseline
        super(IntegratedGradients, self).__init__(T, X, session, keras_learning_phase=keras_learning_phase,A=A)
...
class EpsilonLRP(GradientBasedMethod):
    ...
    def __init__(self, T, X, session, keras_learning_phase=None,A=None, epsilon=1e-4):
        assert epsilon > 0.0, 'LRP epsilon must be greater than zero'
        global eps
        eps = epsilon
        super(EpsilonLRP, self).__init__(T, X, session, A=A, keras_learning_phase=keras_learning_phase)
    ...
    def get_symbolic_attribution(self):
        ...
        return [g * x for g, x in zip(
            tf.gradients(self.T, self.A),
            self.A if self.has_multiple_inputs else [self.A])]
...
class DeepLIFTRescale(GradientBasedMethod):
    ...
    def __init__(self, T, X, session, keras_learning_phase,A=None,baseline=None):
        self.baseline = baseline
        super(DeepLIFTRescale, self).__init__(T, X, session, A=A, keras_learning_phase=keras_learning_phase)
...
class DeepExplain(object):
    ...
    def get_explainer(self, method, T, X, A=None, **kwargs):
    ...
    def explain(self, method, T, X, xs, A=None,ys=None, batch_size=None, **kwargs):
        explainer = self.get_explainer(method, T, X, A=A, **kwargs)
